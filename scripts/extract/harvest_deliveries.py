...
def planned_from_application(grants_root, key):
    """Return (fte, duration_months, cost_usd, application_path) or blanks."""
    for apf in glob.glob(os.path.join(grants_root, "applications", "*.md")):
        if slug(os.path.basename(apf)[:-3]) == key:
            ...
            return (f.group(1) if f else "",
                    ...
                    "github.com/w3f/Grants-Program/blob/master/applications/" + os.path.basename(apf))
    return ("", "", "", "")


def pick_primary(all_repos, key):
    """Deterministic primary-repo choice among project-controlled repos.
    Returns (owner_repo or '', own_org_repos, shared_repos)."""
    own, shared = [], []
    for (o, r) in all_repos:
        (shared if o.lower() in SHARED_OWNERS else own).append(f"{o}/{r}")
    if not own:
        return "", own, shared
    # 1) slug of repo name matches project key
    for orr in own:
        if slug(orr.split("/")[1]) == key or key in slug(orr.split("/")[1]):
            return orr, own, shared
    # 2) slug of owner matches project key
    for orr in own:
        if slug(orr.split("/")[0]) == key or key in slug(orr.split("/")[0]):
            return orr, own, shared
    # 3) most frequent own-org repo (stable: first by appearance already deduped) -> first
...
def main():
    ...
    if not have_g:
        print("[WARN] grants repo clone failed; planned fields will be blank.")

    files = [f for f in glob.glob(os.path.join(deliv, "deliveries", "*.md"))
             if not os.path.basename(f).startswith(".")
             and "template" not in os.path.basename(f).lower()]

    # group delivery files by project key
    groups = {}
    for f in files:
        groups.setdefault(project_key(os.path.basename(f)), []).append(f)
    ...
    for key in sorted(groups):
        ...
        fte, dur, cost, appurl = planned_from_application(grants, key) if have_g else ("", "", "", "")
        planned_pm = ""
        try:
            if fte and dur:
        ...
        audit_rows.append(dict(project_id=key, verdict=verdict, reason=reason,
        ...
        if verdict == "INCLUDED":
            manifest_rows.append(dict(
                project_id=key, project_name=key, repo_url="https://github.com/" + primary,
                ...
                notes="census auto-harvested; primary=slug/own-org rule"))
