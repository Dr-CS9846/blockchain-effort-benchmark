...
def _numstat(d, gitargs):
    ...
    for ln in r.stdout.splitlines():
        ...
        a,dl,path = parts
        if a in ("-","") or not _is_src(path) or _is_gen_path(path): continue
...
# path/name patterns for GENERATED source (PAPI descriptors, scale/protobuf codegen, *.gen.*, *_pb.*)
PATH_GEN = re.compile(r"(^|/)(\.papi|descriptors?|generated|__generated__|codegen|gen)/|"
                      r"\.(gen|generated|g)\.[a-z0-9]+$|(_pb|\.pb)\.[a-z0-9]+$", re.I)
BULK_LOC = 4000   # a single commit adding more source lines than this = import/fork/vendor, not authoring

def _is_gen_path(rel): return bool(PATH_GEN.search(rel))
...
def reuse_split(d):
    """Return (adapted_fraction, info). ADAPTED = generated (markers OR path) + forked/imported
    (files first ADDED in any 'bulk-import' commit that dumped > BULK_LOC source lines at once -
    detected ANYWHERE in history, not just the root). equiv = new + AAM*adapted (applied in dissect)."""
    # 1) find files originating in bulk-import commits (one git-log pass over add-events)
    bulk_files = set(); bulk_commits = 0
    cur_add = 0; cur_files = []
    def _flush():
        nonlocal cur_add, cur_files, bulk_commits
        if cur_add > BULK_LOC:
            bulk_files.update(cur_files); bulk_commits += 1
        cur_add = 0; cur_files = []
    out = sh(["git","log","--diff-filter=A","--numstat","--pretty=tformat:__C__"], cwd=d).stdout
    for ln in out.splitlines():
        if ln.startswith("__C__"): _flush(); continue
        p = ln.split("\t")
        if len(p) == 3 and _is_src(p[2]) and not _is_gen_path(p[2]):
            try: cur_add += int(p[0]); cur_files.append(p[2])
            except ValueError: pass
    _flush()
    # 2) classify every surviving source file
    ...
    return frac, dict(new_phys=new_p, adapted_phys=adapted_p, generated_phys=gen_p, forked_phys=fork_p,
                      bulk_import_commits=bulk_commits, bulk_import_files=len(bulk_files))
