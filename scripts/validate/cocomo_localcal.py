...
def build_features(cand):
    ...
    # CANONICAL COCOMO II size: reuse-adjusted equivalent SLOC (gap #9). Falls back to ksloc where
    # equivalent_sloc is missing (equiv ~= ksloc post vendor-exclusion); competes with ln_ksloc.
    if any((_f(t[0],"equivalent_sloc") or 0) > 0 for t in cand):
        feats["ln_equiv_sloc"]=np.array([math.log(max(_f(t[0],"equivalent_sloc") or t[2],1e-6)) for t in cand]); prosp["ln_equiv_sloc"]=True
