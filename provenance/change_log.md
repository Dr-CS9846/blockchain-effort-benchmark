...
- Next: push -> dispatch functional-size-eval -> read result -> compare prospective functional-size SA/PRED vs KSLOC baseline (PM=PM). If FS matches/beats KSLOC, it becomes the usable estimator + we write functional_size_spec.md (developer counting table) and the calibrated equation. Step 3 latent target still planned as the cleaner calibration target.
