...
- Next: push -> dispatch treasury-mine -> read coverage (total / with-repo / with-plan / useful=repo&plan). If a useful pool exists, build the match+measure step (measure those repos, join planned effort) and recalibrate size->planned_pm at larger n. If sparse, treasury planned-effort is still a standalone reported-effort dataset for the model.
