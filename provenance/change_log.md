...
- Next: push -> re-dispatch -> full harvest (hundreds of proposals). Then build the MATCH+MEASURE step: take proposals with GitHub repos, measure those repos (size) + carry the proposal's requested amount / stated effort -> grow the size->planned/cost calibration set; keep repo-less proposals with stated effort as standalone reported-effort anchors. (Effort-signal regexes FTE/team/duration are conservative; can enrich later - content is captured so re-parse is cheap.)
